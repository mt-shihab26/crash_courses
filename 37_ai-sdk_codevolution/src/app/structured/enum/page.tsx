...
const EnumPage = () => {
    ...
    return (
        <div className="flex flex-col w-full max-w-xl py-24 mx-auto content-stretch">
            <div className="mb-8">
                <h1 className="text-3xl font-bold mb-4">Pokemon Type Classifier</h1>
                <p className="text-gray-600">Enter a Pokemon name to classify its type using AI-generated enum values.</p>
            </div>
            ...
            {result && (
                <div className="mb-6 p-4 bg-zinc-50 rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-white font-medium ${result === 'fire' ? 'bg-red-500' : result === 'water' ? 'bg-blue-500' : result === 'grass' ? 'bg-green-500' : result === 'electric' ? 'bg-yellow-500' : result === 'psychic' ? 'bg-purple-500' : 'bg-gray-500'}`}>{result}</span>
                    </div>
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="pokemon-input" className="block text-sm font-medium mb-2">
                        Pokemon name:
                    </label>
                    <input
                        id="pokemon-input"
                ...
                <button type="submit" disabled={isLoading || !pokemonName.trim()} className="w-full bg-blue-500 text-white px-4 py-3 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-600">
