...
import { recipeSchema } from '../api/structured/schema';
        ...
        <div className="flex flex-col w-full max-w-md py-24 mx-auto content-stretch">
                        ...
                        <div>
                            ...
                            <ol className="space-y-4">
                                {object.recipe.steps.map((step, index) => (
                                    <li key={index} className="bg-zinc-50 p-4 rounded-lg">
                                        <span className="font-medium mr-2">{index + 1}. </span> {step}
                                    </li>
                                ))}
                            </ol>
                        </div>
            ...
            >
                <div className="flex gap-2">
                    <input placeholder="Write your dish name?" className="flex-1 p-2 border border-zinc-300" value={dish} onChange={e => setDish(e.target.value)} />
                    {isLoading && (
                        <button onClick={stop} className="bg-red-500 text-white px-4 py-2 rounded transition">
                            Stop
                        </button>
                    )}
                    <button disabled={isLoading} type="submit" className="bg-blue-500 text-white px-4 py-2 rounded transition">
                        Send
                    </button>
                </div>
            </form>
